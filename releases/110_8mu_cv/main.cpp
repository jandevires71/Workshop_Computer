...
// card turns the 8mu's six lefthand faders into four control voltages and two
// square-wave pulse streams:
//
...
//   fader 5  ->  Pulse Out 1   rate, 0.1Hz to 20Hz square, 50% duty
//   fader 6  ->  Pulse Out 2   rate, 0.1Hz to 20Hz square, 50% duty
//
...
public:
	...
	{
		// --- where do the six parameters come from this sample? -------------
		//
		...
		// --- two square-wave pulse streams ----------------------------------
		//
		// A 32-bit phase accumulator: add a per-sample increment and read the
		// top bit, which gives an exact 50% duty square.  The increment table
		// covers 0.1Hz to 20Hz, and because the faders are 7-bit it has one
		// entry per possible CC value.
		phase[0] += rateInc[param[4] >> 5];
		phase[1] += rateInc[param[5] >> 5];
		PulseOut1((phase[0] & 0x80000000u) != 0);
		PulseOut2((phase[1] & 0x80000000u) != 0);
...
private:
	...
	int32_t param[6] = {2048, 2048, 2048, 2048, 0, 0};
